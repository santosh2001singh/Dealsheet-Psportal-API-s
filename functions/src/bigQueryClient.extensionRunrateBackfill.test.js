...
test("applyExtensionInheritForInsertRows also merges runrate-matched hierarchy *_EMP_NO fields (resolved by name+designation against directory_employees, not from runrate itself)", async () => {
  const rows = [
    {
      DEAL_TYPE: "EXTENSION",
      CONTRACT_ID: null,
...
test("applyExtensionInheritForInsertRows skips non-EXTENSION rows", async () => {
  ...
  const out = await applyExtensionInheritForInsertRows(rows, {}, { parentFetchFn, runrateFetchFn });
