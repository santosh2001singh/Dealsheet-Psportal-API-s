...
//                has none/blank.
//   TENTATIVE_DATE — the JOB's end_date ONLY (no submittal fallback).
...
test("TENTATIVE_DATE uses job.end_date only (no submittal fallback)", () => {
  ...
  assert.equal(
    mapJobSubmittalToBq({ id: 1, end_date: "2026-06-08" }, {}).TENTATIVE_DATE,
    null
  );
});
