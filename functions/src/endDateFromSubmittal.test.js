...
// Business rule:
//  TENTATIVE_DATE = job.end_date  (planned/tentative end)
...
test("END_DATE is null while STARTED / BOOKED / ACTIVE (not ended yet — planned end lives in TENTATIVE_DATE)", () => {
...
test("TENTATIVE_DATE = job.end_date (planned), START_DATE = job.start_date", () => {
  const m = mapJobSubmittalToBq(withStatus("EARLY_TERM"), JOB);
  assert.equal(m.TENTATIVE_DATE, "2026-09-19");
